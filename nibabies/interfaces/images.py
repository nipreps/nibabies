...
class RemapLabelsInputSpec(BaseInterfaceInputSpec):
    ...
    dict = traits.Dict(
        exists=True,
        mandatory=True,
        key_trait=traits.BaseInt,
        value_trait=traits.BaseInt,
        desc="key-value pairs for remapping",
    )
...
class RemapLabels(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        self._results['out_file'] = fname_presuffix(
            self.inputs.in_file, suffix='_remapped', newpath=runtime.cwd
        )
        ...
        nb.Nifti1Image(out_data.astype(int), in_img.affine, in_img.header).to_filename(
