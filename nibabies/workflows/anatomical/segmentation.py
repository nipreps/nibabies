...
def init_aseg_to_mcrib_wf(omp_nthreads=1, name="aseg_to_mcrib_wf"):
    ...
    outputnode = pe.Node(
        niu.IdentityInterface(fields=["anat_mcrib"]),
        name="outputnode",
    )
    ...
    remap_labels = pe.Node(RemapLabels(dict=aseg2mcrib), name="remap_labels")

    # fmt:off
    wf.connect([
        (inputnode, remap_labels, [("anat_aseg", "in_file")]),
        (remap_labels, outputnode, [("out_file", "anat_mcrib")]),
    ])
    # fmt:on

    # TODO Pass in the anat_mcrib as the segmentation for the MCRIB-based reconall in base.py
    return wf
